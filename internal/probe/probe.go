...
func ProbeVolter(addr string, wireToken string, timeout time.Duration) (ok bool, ipv6 bool, err error) {
	ok, ipv6, _, err = ProbeVolterWithCaps(addr, wireToken, timeout)
	return ok, ipv6, err
}

func ProbeVolterWithCaps(addr string, wireToken string, timeout time.Duration) (ok bool, ipv6 bool, caps *protocol.ServerHelloCaps, err error) {
	if timeout <= 0 {
		timeout = defaultProbeTimeout
	}
