...
func TestProbeVolterRefused(t *testing.T) {
	l, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		t.Skip("no tcp:", err)
	}
	...
	ok, _, err := ProbeVolter(addr, "unused", 100*time.Millisecond)
	if err == nil || ok {
		t.Errorf("want err, got ok=%v err=%v", ok, err)
	}
}

func TestProbeVolterCloseOnBadToken(t *testing.T) {
	l, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		t.Skip("no tcp:", err)
	}
	...
	ok, _, err := ProbeVolter(l.Addr().String(), "any", time.Second)
	if err != nil {
		t.Fatalf("err=%v", err)
	}
	if !ok {
...
func TestProbeVolterServerRejectsBadToken(t *testing.T) {
	...
	ok, _, err := ProbeVolter(l.Addr().String(), serverToken, time.Second)
	if err != nil {
		t.Fatalf("err=%v", err)
	}
	if !ok {
