...
async function createClient() { let body = { name: $('newName').value, groupId: $('newGroup').value, note: $('newNote').value, expiresAt: Number($('newExp').value || 0) }; let r = await api('/api/v1/clients', { method: 'POST', body: JSON.stringify(body) }); showVoultKey(r.voultkey); toast('ok'); loadAll(); }
async function rotate(id) { let r = await api(`/api/v1/clients/${id}/rotate`, { method: 'POST', body: '{}' }); showVoultKey(r.voultkey); toast('ok'); }
