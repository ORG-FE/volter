...
final class TcpReactorPool {
  ...
  private static final class Reactor {
    ...
    private final ConcurrentLinkedQueue<Registration> pending = new ConcurrentLinkedQueue<>();
    ...
    private void drainPending() {
      while (true) {
        ...
        try {
          ...
          Session session = new Session(reg.xor(), reg.initialClientData(), reg.initialClientDataDecrypted());
          session.setClientKey(client.register(selector, SelectionKey.OP_READ, session));
    ...
    private static final class Session {
      private static final long CONNECT_TIMEOUT_NANOS = TimeUnit.MILLISECONDS.toNanos(
        CONNECT_TIMEOUT_MS
      );
