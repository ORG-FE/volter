...
func setErr(s *session, errStr string) {
	...
	s.err = errStr
	fmt.Fprintf(os.Stderr, "volter-core: %s\n", errStr)
}
...
func StartTun(tunFd int, mtu int, cfgJSON string, configDir string) string {
	...
	probeOK, _, probeCaps, _ := probe.ProbeVolterWithCaps(addrs[0], cfg.Token, 10*time.Second)
	if cfg.Protection != nil && cfg.Protection.PreCheck && !probeOK {
		return jsonString(startResult{Handle: 0, Error: "preCheck: server is not volter"})
	}
...
func StartStandaloneDpi(cfgJSON string, configDir string) string {
	...
	dopts := dpiengine.Options{
		...
		FakeSNI:       emb.FakeSNI,
		FakeSNIHost:   emb.FakeSNIHost,
		SplitPosition: emb.SplitPosition,
		AutoTTL:       emb.AutoTTL,
		TCPSegment:    emb.TCPSegment,
		OOBData:       emb.OOBData,
		MultiSplit:    emb.MultiSplit,
	}
...
func ProbeVolter(server string, token string, timeoutMs int) string {
	...
	ok, ipv6, caps, err := probe.ProbeVolterWithCaps(server, token, time.Duration(timeoutMs)*time.Millisecond)
	if err != nil {
